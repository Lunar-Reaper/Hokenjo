...
--		"adv/vita",			-- Vita専用
		...
		"adv/adv",			-- ADV制御
		"adv/delay",		-- delay制御
		"adv/language",		--(AD):多言語制御
		"adv/sceneskip",	-- シーンスキップ制御
...
--		"ui/flow",			-- フローチャート制御
	...
	-- 機種判定
	local tbl = {
		--						PS		CS		SP		touch				PS		CS
		windows = { fakeos = {	nil,	nil,	nil,	true }, trueos = {	nil,	nil  } },
		android = { fakeos = {	nil,	nil,	true,	true }, trueos = {	nil,	nil  } },
		ios		= { fakeos = {	nil,	nil,	true,	true }, trueos = {	nil,	nil  } },
		ps4		= { fakeos = {	true,	true,	nil,	nil  }, trueos = {	true,	true } },
		vita	= { fakeos = {	true,	true,	nil,	true }, trueos = {	true,	true } },
		switch	= { fakeos = {	nil,	true,	nil,	true }, trueos = {	nil,	true } },
		wasm	= { fakeos = {	nil,	nil,	true,	true }, trueos = {	nil,	nil  } },
	}
	local gos = game.os
	...
	-- wasm専用
	if tos == "wasm" then
		e:tag{"callnative", method = "navigator.platform;" , result = "t.wasm.os"}
		e:tag{"callnative", method = "navigator.userAgent;", result = "t.wasm.ua"}
		local pf = string.lower(e:var("t.wasm.os"))		-- OS名
		local ua = string.lower(e:var("t.wasm.ua"))		-- UA
		local s2 = pf:sub(1, 2)
		...
		elseif s2 == 'li' then pf = 'android' end		-- androidはLinux armになる
			if ua:find('edge')	  then ua = 'edge'		-- Edge  はSafariとChromeを含む
...
function system_dataloading()
	...
	if not conf then
		flg.firstboot = true --(AD)：初回起動のフラグ
...
--		e:include("system/dmmck.lua")		-- DMM認証
...
function set_uipath()
	...
	local px = init.lang[ln]
	game.path.ui = init.system.ui_path..px
end
----------------------------------------
